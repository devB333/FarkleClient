...
export function Game({ clientConn }) {
  ...
  useEffect(() => {
    function updateScale() {
      ...
      setScale(Math.min(scaleX, scaleY)); // shrink to whichever axis is tighter
    }
    updateScale();
  ...
  */
  ...
  function renderBankedDice() {
    ...
    return bankedDice.map((die) => (
      <div key={die.id * -1} style={{
        margin: '0 0.5%', display: 'flex', justifyContent: 'center', height: '175%', aspectRatio: '1/1',
  ...
  useEffect(() => {// loop timer for popOffsetLoop
